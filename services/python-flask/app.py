...
public_key: VerifyKey | None = None
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"Created topic: {pubsub_topic_path}")
                except Exception as create_err:
                    logger.warning(f"Failed to create topic: {create_err}")

            logger.info(f"Pub/Sub configured: {pubsub_topic_path}")
        except Exception as e:
...
def handle_interaction():
    ...
    if interaction_type == INTERACTION_TYPE_PING:
        ...
        return handle_application_command(interaction)
    else:
...
def handle_ping():
    ...
    return jsonify({"type": RESPONSE_TYPE_PONG})


def handle_application_command(interaction: dict):
    ...
    return jsonify({"type": RESPONSE_TYPE_DEFERRED_CHANNEL_MESSAGE})
